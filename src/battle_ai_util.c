...
static enum AIScore IncreaseStatUpScoreInternal(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum StatChange statChange, bool32 considerContrary)
{
    ...
    // If not switching, don't pile on setup while the target is already boosting
    if (!IsBattlerPredictedToSwitch(battlerDef) && CountPositiveStatStages(battlerDef) >= 2)
        return NO_INCREASE;
