...
EWRAM_DATA u8 gDisplayedStringBattle[380] = {0}; // Increased in size to fit Brock's defeat text, which is 375 characters long (PewterCity_Gym_Text_BrockDefeat)
...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    [TRAINER_CLASS_RS_POKEMANIAC] =   { _("Poké Maniac"), 15 },
    ...
    [TRAINER_CLASS_POKEMANIAC] =      { _("Poké Maniac"), 12 },
...
static void DoBattleIntro(void)
{
    ...
    case BATTLE_INTRO_STATE_WAIT_FOR_WILD_BATTLE_TEXT:
        if (!IsBattlerMarkedForControllerExec(GetBattlerAtPosition(B_POSITION_PLAYER_LEFT)))
        {
            if (gBattleTypeFlags & BATTLE_TYPE_GHOST && (CheckBagHasItem(ITEM_SILPH_SCOPE, 1) || FlagGet(FLAG_DELIVERED_SILPH_SCOPE)))
            {
