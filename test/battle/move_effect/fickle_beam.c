...
SINGLE_BATTLE_TEST("Fickle Beam deals double damage 30% of the time")
{
    ...
    GIVEN {
        ASSUME(GetMovePower(MOVE_DAZZLE_GLEAM) == GetMovePower(MOVE_FICKLE_BEAM));
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(player, MOVE_DAZZLE_GLEAM); }
        TURN { MOVE(player, MOVE_FICKLE_BEAM); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_DAZZLE_GLEAM, player);
